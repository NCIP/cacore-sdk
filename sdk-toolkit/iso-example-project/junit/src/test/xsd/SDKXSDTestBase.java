...
public abstract class SDKXSDTestBase extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		usePermissibleValues = Boolean.parseBoolean(System
				.getProperty("usePermissibleValues"));
	...
	public void validateClassElements(Class klass, String klassAlias)
		...
		if (!(null == klassElt.getAttribute("abstract"))
				&& klassElt.getAttributeValue("abstract").equals("true")) {
	 ...
	 * @throws Exception
	 */
	public void validateClassIsAbstract(Class klass) throws Exception {
		Document doc = getDoc();
	 ...
	 * @throws Exception
	 */
	public void validateSubclassElements(Class klass) throws Exception {
		validateSubclassElements(klass, klass.getSimpleName(),
				getSuperklassName(klass));
	}

	public void validateSubclassElements(Class klass, String klassAlias,
			String superklassName) throws Exception {
	...
	/**
	 * Uses xpath to query the generated XSD Verifies that Subclass Association
	 * elements (extension, sequence) and attributes (base, ref) are present
	 * 
	 * @throws Exception
	 */
	public void validateClassAssociationElements(Class klass,
			Class associatedKlass, String rolename, String minOccurs,
			String maxOccurs) throws Exception {
		validateClassAssociationElements(klass, klass.getSimpleName(),
				associatedKlass, associatedKlass.getSimpleName(), rolename,
				minOccurs, maxOccurs);
	}

	public void validateClassAssociationElements(Class klass,
			String klassAlias, Class associatedKlass,
			String associatedKlassAlias, String rolename, String minOccurs,
			String maxOccurs) throws Exception {
		...
		Element klassElt = elts.get(0);
		//assertEquals(4, klassElt.getAttributes().size()); 
	 ...
	 * @throws Exception
	 */
	public void validateSubclassAssociationElements(Class klass,
			Class associatedKlass, String rolename, String minOccurs,
			String maxOccurs) throws Exception {
		validateSubclassAssociationElements(klass, klass.getSimpleName(),
				associatedKlass, associatedKlass.getSimpleName(), klass
						.getSuperclass().getSimpleName(), rolename, minOccurs,
				maxOccurs);
	}

	public void validateSubclassAssociationElements(Class klass,
			String klassAlias, Class associatedKlass,
			String associatedKlassAlias, String superklassName,
			String rolename, String minOccurs, String maxOccurs)
			throws Exception {
	...
	protected void validateAttributeElement(Class klass, String attributeName,
			String attributeType) throws Exception {
		validateAttributeElement(klass, klass.getSimpleName(), attributeName,
				attributeType);
	}
	...
	protected void validateAttributeWithRestriction(Class klass,
			String attributeName, String attributeType, String permissibleValue)
			throws Exception {
		...
		assertEquals(restrictionElt.getAttributeValue("base").toLowerCase(),
				"xs:" + attributeType.toLowerCase());
		...
		assertEquals(enumerationElt.getAttributeValue("value"),
				permissibleValue);
	}

	protected void validateSubclassAttributeElement(Class klass,
			String attributeName, String attributeType) throws Exception {
		validateSubclassAttributeElement(klass, klass.getSimpleName(),
				getSuperklassName(klass), attributeName, attributeType);
	}

	protected void validateSubclassAttributeElement(Class klass,
			String klassAlias, String superklassName, String attributeName,
			String attributeType) throws Exception {
	...
	private String getSuperklassName(Class klass) {
		...
		if (klass.getPackage().getName().equals(
				klass.getSuperclass().getPackage().getName()))
			return klass.getSuperclass().getSimpleName();

		return klass.getSuperclass().getPackage().getName() + ":"
				+ klass.getSuperclass().getSimpleName();
	}

	private Element locateChild(List<Element> eltList, String permissibleValue) {
		for (Element elt : eltList) {
			log.debug("* * * " + elt.getAttributeValue("value")
