...
/*
 * Test case for TelPhone (TEL.PHONE)
 */
public class TelPhoneDataTypeTest extends SDKISOTestBase
{
	...
	{
		return "Tel Phone Datatype Test Case";
	}
	...
	{
		TelPhoneDataType searchObject = new TelPhoneDataType();
		Collection results = search("gov.nih.nci.cacoresdk.domain.other.datatype.TelPhoneDataType",searchObject );
	...
	{
		HQLCriteria criteria = new HQLCriteria("from gov.nih.nci.cacoresdk.domain.other.datatype.TelPhoneDataType");
		int count = getApplicationService().getQueryRowCount(criteria, "gov.nih.nci.cacoresdk.domain.other.datatype.TelPhoneDataType");
	...
	private void assertValue1(Collection<TelPhoneDataType> result, List<Integer> index) throws URISyntaxException
	{
		...
		for(TelPhoneDataType data : result)
		{
			...
			else if((index == null && counter == 3) || (index != null && index.contains("3")))
			{
				...
				assertEquals(new URI("x-text-tel:8004226232"), data.getValue1().getValue());
				assertValue1Constants(data);
			...
			else if((index == null && counter == 4) || (index != null && index.contains("4")))
			{
				...
				assertEquals(new URI("x-text-fax:8004226233"), data.getValue1().getValue());
				assertValue1Constants(data);
			...
			else if((index == null && counter == 5) || (index != null && index.contains("5")))
			{
				...
				assertEquals(new URI("tel:8004226234"), data.getValue1().getValue());
				assertValue1Constants(data);
			...
			else if((index == null && counter == 6) || (index != null && index.contains("6")))
			{
				...
				assertEquals(new URI("tel:8004226235"), data.getValue1().getValue());
				assertValue1Constants(data);
			...
			else
			{
				...
				assertEquals(NullFlavor.NA, data.getValue1().getNullFlavor());				counter++;
			}
		}
	}

	private void assertValue1Constants(TelPhoneDataType data)
	{
		assertNull(data.getValue1().getNullFlavor());
	}
	
	private void assertValue2(Collection<TelPhoneDataType> result, List<Integer> index) throws URISyntaxException
	{
		assertNotNull(result);
		...
		for(TelPhoneDataType data : result)
		{
			...
			if((index == null && counter == 7) || (index != null && index.contains("7")))
			{
				...
				assertEquals(new URI("tel:8004226231"), data.getValue2().getValue());
				//From the database
			...
			else if((index == null && counter == 8) || (index != null && index.contains("8")))
			{
				...
				assertEquals(new URI("x-text-tel:8004226232"), data.getValue2().getValue());
				//From the database
				assertNull(data.getValue2().getNullFlavor());
			...
			else if((index == null && counter == 11) || (index != null && index.contains("11")))
			{
				...
				assertNull(data.getValue2().getNullFlavor());
				assertEquals(new URI("tel:8004226235"), data.getValue2().getValue());
