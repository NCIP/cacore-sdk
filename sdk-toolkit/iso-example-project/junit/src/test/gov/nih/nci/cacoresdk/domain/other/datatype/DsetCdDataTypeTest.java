	...
	private void assertValue1(Collection<DsetCdDataType> result, List<Integer> index)
	{
		...
		for(DsetCdDataType data : result)
		{
			...
			if((index == null && counter == 1) || (index != null && index.contains("1")))
			{
				...
				assertNull(item.getDisplayName().getValue());
				assertEquals(NullFlavor.NI, item.getDisplayName().getNullFlavor());
				assertNotNull(item.getOriginalText());
				assertEquals(NullFlavor.NI, item.getOriginalText().getNullFlavor());
				assertNull(item.getOriginalText().getValue());
