...
public class TestRESTClient
{
	...
	public void testSearch() throws Exception
	{
		...
		String baseUri = "http://localhost:21080/example/rest/";
		...
		for(Class klass:classList)
		{
			if (!Modifier.isAbstract(klass.getModifiers())){
				...
				try
				{
					...
					String pathName = klass.getName().substring(klass.getName().lastIndexOf(".")+1, klass.getName().length());
					client.type("application/xml").accept("application/xml");
					client.path(pathName);
					Object obj = client.path("1").get(klass);
					printObject(obj, klass, true);
				}catch(Exception e)
				{
