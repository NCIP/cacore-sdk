...
public class PendantResourceTest extends SDKRESTfulTestBase
{
	public static String getTestCaseName()
	{
		return "Pendant  RESTful Resource Test Case";
	}
	...
	public void testGet() throws Exception
	{
	  ...
	  try {
		...
		{
			Pendant obj = (Pendant) ((List)results).get(0);
		...
		String url = baseURL + "/rest/Pendant/"+id;
		...
		if (response.getStatus() == Status.NOT_ACCEPTABLE.getStatusCode()) {
 		...
 		File myFile = new File("Pendant"+"XML.xml");	
	...
	public void testSearch() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/Pendant/search;id=*";
		WebClient client = WebClient.create(url);
		client.type("application/xml").accept("application/xml");		
		Response response = client.get();
		...
		if (response.getStatus() == Status.NOT_ACCEPTABLE.getStatusCode()) {
 		...
 		File myFile = new File("Pendant_Search"+"XML.xml");						
		System.out.println("writing data to file "+myFile.getAbsolutePath());
		FileWriter myWriter = new FileWriter(myFile);
...
//***************************************************
	...
	public void testPendant1()
	{
		try
		{
	
		String url = baseURL + "/rest/Pendant/search;id=*/chain";
	...
	public void testgetChain()
	{
		...
		{
		Pendant searchObject = new Pendant();
 		Collection results5 = getApplicationService().search("gov.nih.nci.cacoresdk.domain.onetoone.bidirectional.withjoin.Pendant",searchObject );
		String id = "";
		
		if(results5 != null && results5.size() > 0)
		{
			Pendant obj = (Pendant) ((List)results5).get(0);
		...
		if(id.equals(""))
			...
			String url = baseURL + "/rest/Pendant/"+id+"/chain";
			WebClient client = WebClient.create(url);
			client.type("application/xml").accept("application/xml");		
			Response response = client.get();
...
//********************************************************End
	...
	public void testPost() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/Pendant";
		WebClient client = WebClient.create(url);
		...
		File myFile = new File("Pendant"+"XML.xml");						
 		...
 		{
 			testGet();
 			myFile = new File("Pendant"+"XML.xml");
 			if(!myFile.exists())
				return;
 		}
