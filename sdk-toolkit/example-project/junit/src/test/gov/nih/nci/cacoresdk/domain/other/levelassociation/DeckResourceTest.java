...
public class DeckResourceTest extends SDKRESTfulTestBase
{
	public static String getTestCaseName()
	{
		return "Deck  RESTful Resource Test Case";
	}
	...
	public void testGet() throws Exception
	{
	  ...
	  try {
		...
		{
			Deck obj = (Deck) ((List)results).get(0);
		...
		String url = baseURL + "/rest/Deck/"+id;
		...
		if (response.getStatus() == Status.NOT_ACCEPTABLE.getStatusCode()) {
 		...
 		File myFile = new File("Deck"+"XML.xml");	
	...
	public void testSearch() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/Deck/search;id=*";
		WebClient client = WebClient.create(url);
		client.type("application/xml").accept("application/xml");		
		Response response = client.get();
...
//***************************************************
	...
	public void testgetSuitCollection()
	{
		try
		{
		Deck searchObject = new Deck();
 		Collection results4 = getApplicationService().search("gov.nih.nci.cacoresdk.domain.other.levelassociation.Deck",searchObject );
		String id = "";
		
		if(results4 != null && results4.size() > 0)
		{
			Deck obj = (Deck) ((List)results4).get(0);
		...
		if(id.equals(""))
			...
			String url = baseURL + "/rest/Deck/"+id+"/suitCollection";
