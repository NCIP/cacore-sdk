...
public class LCDMonitorResourceTest extends SDKRESTfulTestBase
{
	public static String getTestCaseName()
	{
		return "LCDMonitor  RESTful Resource Test Case";
	}
	...
	public void testGet() throws Exception
	{
	  ...
	  try {
		...
		{
			LCDMonitor obj = (LCDMonitor) ((List)results).get(0);
		...
		String url = baseURL + "/rest/LCDMonitor/"+id;
		...
		if (response.getStatus() == Status.NOT_ACCEPTABLE.getStatusCode()) {
 		...
 		File myFile = new File("LCDMonitor"+"XML.xml");	
	...
	public void testSearch() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/LCDMonitor/search;id=*";
		WebClient client = WebClient.create(url);
		client.type("application/xml").accept("application/xml");		
		Response response = client.get();
		...
		if (response.getStatus() == Status.NOT_ACCEPTABLE.getStatusCode()) {
 		...
 		File myFile = new File("LCDMonitor_Search"+"XML.xml");						
		System.out.println("writing data to file "+myFile.getAbsolutePath());
		FileWriter myWriter = new FileWriter(myFile);
...
//********************************************************End
	...
	public void testPost() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/LCDMonitor";
		WebClient client = WebClient.create(url);
		...
		File myFile = new File("LCDMonitor"+"XML.xml");						
 		...
 		{
 			testGet();
 			myFile = new File("LCDMonitor"+"XML.xml");
 			if(!myFile.exists())
				return;
 		}
