...
public class SoftTopResourceTest extends SDKRESTfulTestBase
{
	public static String getTestCaseName()
	{
		return "SoftTop  RESTful Resource Test Case";
	}
	...
	public void testGet() throws Exception
	{
	  ...
	  try {
		...
		{
			SoftTop obj = (SoftTop) ((List)results).get(0);
		...
		String url = baseURL + "/rest/SoftTop/"+id;
		...
		if (response.getStatus() == Status.NOT_ACCEPTABLE.getStatusCode()) {
 		...
 		File myFile = new File("SoftTop"+"XML.xml");	
	...
	public void testSearch() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/SoftTop/search;id=*";
		WebClient client = WebClient.create(url);
		client.type("application/xml").accept("application/xml");		
		Response response = client.get();
		...
		if (response.getStatus() == Status.NOT_ACCEPTABLE.getStatusCode()) {
 		...
 		File myFile = new File("SoftTop_Search"+"XML.xml");						
		System.out.println("writing data to file "+myFile.getAbsolutePath());
		FileWriter myWriter = new FileWriter(myFile);
...
//***************************************************
	...
	public void testSoftTop1()
	{
		try
		{
	
		String url = baseURL + "/rest/SoftTop/search;id=*/wheel";
	...
	public void testgetWheel()
	{
		...
		{
		SoftTop searchObject = new SoftTop();
 		Collection results5 = getApplicationService().search("gov.nih.nci.cacoresdk.domain.inheritance.parentwithassociation.sametable.SoftTop",searchObject );
		String id = "";
		
		if(results5 != null && results5.size() > 0)
		{
			SoftTop obj = (SoftTop) ((List)results5).get(0);
		...
		if(id.equals(""))
			...
			String url = baseURL + "/rest/SoftTop/"+id+"/wheel";
			WebClient client = WebClient.create(url);
			client.type("application/xml").accept("application/xml");		
			Response response = client.get();
...
//********************************************************End
	...
	public void testPost() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/SoftTop";
		WebClient client = WebClient.create(url);
		...
		File myFile = new File("SoftTop"+"XML.xml");						
 		...
 		{
 			testGet();
 			myFile = new File("SoftTop"+"XML.xml");
 			if(!myFile.exists())
				return;
 		}
