...
public class LuggageResourceTest extends SDKRESTfulTestBase
{
	public static String getTestCaseName()
	{
		return "Luggage  RESTful Resource Test Case";
	}
	...
	public void testGet() throws Exception
	{
	  ...
	  try {
		...
		{
			Luggage obj = (Luggage) ((List)results).get(0);
		...
		String url = baseURL + "/rest/Luggage/"+id;
		...
		if (response.getStatus() == Status.NOT_ACCEPTABLE.getStatusCode()) {
 		...
 		File myFile = new File("Luggage"+"XML.xml");	
	...
	public void testSearch() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/Luggage/search;id=*";
		WebClient client = WebClient.create(url);
		client.type("application/xml").accept("application/xml");		
		Response response = client.get();
...
//***************************************************
	...
	public void testgetWheel()
	{
		...
		{
		Luggage searchObject = new Luggage();
 		Collection results5 = getApplicationService().search("gov.nih.nci.cacoresdk.domain.inheritance.parentwithassociation.sametable.Luggage",searchObject );
		String id = "";
		
		if(results5 != null && results5.size() > 0)
		{
			Luggage obj = (Luggage) ((List)results5).get(0);
		...
		if(id.equals(""))
			...
			String url = baseURL + "/rest/Luggage/"+id+"/wheel";
			WebClient client = WebClient.create(url);
			client.type("application/xml").accept("application/xml");		
			Response response = client.get();
...
//********************************************************End
	...
	public void testPost() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/Luggage";
		WebClient client = WebClient.create(url);
		...
		File myFile = new File("Luggage"+"XML.xml");						
 		...
 		{
 			testGet();
 			myFile = new File("Luggage"+"XML.xml");
 			if(!myFile.exists())
				return;
 		}
