...
public class OrderLineResourceTest extends SDKRESTfulTestBase
{
	public static String getTestCaseName()
	{
		return "OrderLine  RESTful Resource Test Case";
	}
	...
	public void testGet() throws Exception
	{
	  ...
	  try {
		...
		{
			OrderLine obj = (OrderLine) ((List)results).get(0);
		...
		String url = baseURL + "/rest/OrderLine/"+id;
		...
		if (response.getStatus() == Status.NOT_ACCEPTABLE.getStatusCode()) {
 		...
 		File myFile = new File("OrderLine"+"XML.xml");	
	...
	public void testSearch() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/OrderLine/search;id=*";
		WebClient client = WebClient.create(url);
		client.type("application/xml").accept("application/xml");		
		Response response = client.get();
		...
		if (response.getStatus() == Status.NOT_ACCEPTABLE.getStatusCode()) {
 		...
 		File myFile = new File("OrderLine_Search"+"XML.xml");						
		System.out.println("writing data to file "+myFile.getAbsolutePath());
		FileWriter myWriter = new FileWriter(myFile);
...
//***************************************************
	...
	public void testOrderLine1()
	{
		...
		String url = baseURL + "/rest/OrderLine/search;id=*/product";
	...
	public void testgetProduct()
	{
		...
		{
		OrderLine searchObject = new OrderLine();
 		Collection results5 = getApplicationService().search("gov.nih.nci.cacoresdk.domain.onetoone.bidirectional.OrderLine",searchObject );
		String id = "";
		
		if(results5 != null && results5.size() > 0)
		{
			OrderLine obj = (OrderLine) ((List)results5).get(0);
		...
		if(id.equals(""))
			...
			String url = baseURL + "/rest/OrderLine/"+id+"/product";
			WebClient client = WebClient.create(url);
			client.type("application/xml").accept("application/xml");		
			Response response = client.get();
...
//********************************************************End
	...
	public void testPost() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/OrderLine";
		WebClient client = WebClient.create(url);
		...
		File myFile = new File("OrderLine"+"XML.xml");						
 		...
 		{
 			testGet();
 			myFile = new File("OrderLine"+"XML.xml");
 			if(!myFile.exists())
				return;
 		}
