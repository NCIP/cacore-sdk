...
public class AuthorResourceTest extends SDKRESTfulTestBase
{
	public static String getTestCaseName()
	{
		return "Author  RESTful Resource Test Case";
	}
	...
	public void testGet() throws Exception
	{
	  ...
	  try {
		...
		{
			Author obj = (Author) ((List)results).get(0);
		...
		String url = baseURL + "/rest/Author/"+id;
		...
		if (response.getStatus() == Status.NOT_ACCEPTABLE.getStatusCode()) {
 		...
 		File myFile = new File("Author"+"XML.xml");	
	...
	public void testSearch() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/Author/search;id=*";
		WebClient client = WebClient.create(url);
		client.type("application/xml").accept("application/xml");		
		Response response = client.get();
		...
		if (response.getStatus() == Status.NOT_ACCEPTABLE.getStatusCode()) {
 		...
 		File myFile = new File("Author_Search"+"XML.xml");						
		System.out.println("writing data to file "+myFile.getAbsolutePath());
		FileWriter myWriter = new FileWriter(myFile);
...
//********************************************************End
	...
	public void testPost() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/Author";
		WebClient client = WebClient.create(url);
		...
		File myFile = new File("Author"+"XML.xml");						
 		...
 		{
 			testGet();
 			myFile = new File("Author"+"XML.xml");
 			if(!myFile.exists())
				return;
 		}
