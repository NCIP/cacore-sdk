...
public class HardTopTypeResourceTest extends SDKRESTfulTestBase
{
	public static String getTestCaseName()
	{
		return "HardTopType  RESTful Resource Test Case";
	}
	...
	public void testGet() throws Exception
	{
	  ...
	  try {
		...
		{
			HardTopType obj = (HardTopType) ((List)results).get(0);
		...
		String url = baseURL + "/rest/HardTopType/"+id;
		...
		if (response.getStatus() == Status.NOT_ACCEPTABLE.getStatusCode()) {
 		...
 		File myFile = new File("HardTopType"+"XML.xml");	
	...
	public void testSearch() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/HardTopType/search;id=*";
		WebClient client = WebClient.create(url);
		client.type("application/xml").accept("application/xml");		
		Response response = client.get();
...
//***************************************************
	...
	public void testgetWheel()
	{
		...
		{
		HardTopType searchObject = new HardTopType();
 		Collection results5 = getApplicationService().search("gov.nih.nci.cacoresdk.domain.inheritance.parentwithassociation.sametable.HardTopType",searchObject );
		String id = "";
		
		if(results5 != null && results5.size() > 0)
		{
			HardTopType obj = (HardTopType) ((List)results5).get(0);
		...
		if(id.equals(""))
			...
			String url = baseURL + "/rest/HardTopType/"+id+"/wheel";
			WebClient client = WebClient.create(url);
			client.type("application/xml").accept("application/xml");		
			Response response = client.get();
...
//********************************************************End
	...
	public void testPost() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/HardTopType";
		WebClient client = WebClient.create(url);
		...
		File myFile = new File("HardTopType"+"XML.xml");						
 		...
 		{
 			testGet();
 			myFile = new File("HardTopType"+"XML.xml");
 			if(!myFile.exists())
				return;
 		}
