...
public class ProductResourceTest extends SDKRESTfulTestBase
{
	public static String getTestCaseName()
	{
		return "Product  RESTful Resource Test Case";
	}
	...
	public void testGet() throws Exception
	{
	  ...
	  try {
		...
		{
			Product obj = (Product) ((List)results).get(0);
		...
		String url = baseURL + "/rest/Product/"+id;
		...
		if (response.getStatus() == Status.NOT_ACCEPTABLE.getStatusCode()) {
 		...
 		File myFile = new File("Product"+"XML.xml");	
	...
	public void testSearch() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/Product/search;id=*";
		WebClient client = WebClient.create(url);
		client.type("application/xml").accept("application/xml");		
		Response response = client.get();
		...
		if (response.getStatus() == Status.NOT_ACCEPTABLE.getStatusCode()) {
 		...
 		File myFile = new File("Product_Search"+"XML.xml");						
		System.out.println("writing data to file "+myFile.getAbsolutePath());
		FileWriter myWriter = new FileWriter(myFile);
...
//***************************************************
	...
	public void testProduct1()
	{
		...
		String url = baseURL + "/rest/Product/search;id=*/line";
	...
	public void testgetLine()
	{
		...
		{
		Product searchObject = new Product();
 		Collection results5 = getApplicationService().search("gov.nih.nci.cacoresdk.domain.onetoone.bidirectional.Product",searchObject );
		String id = "";
		
		if(results5 != null && results5.size() > 0)
		{
			Product obj = (Product) ((List)results5).get(0);
		...
		if(id.equals(""))
			...
			String url = baseURL + "/rest/Product/"+id+"/line";
			WebClient client = WebClient.create(url);
			client.type("application/xml").accept("application/xml");		
			Response response = client.get();
...
//********************************************************End
	...
	public void testPost() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/Product";
		WebClient client = WebClient.create(url);
		...
		File myFile = new File("Product"+"XML.xml");						
 		...
 		{
 			testGet();
 			myFile = new File("Product"+"XML.xml");
 			if(!myFile.exists())
				return;
 		}
