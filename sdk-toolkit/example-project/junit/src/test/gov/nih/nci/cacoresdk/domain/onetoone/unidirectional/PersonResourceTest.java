...
public class PersonResourceTest extends SDKRESTfulTestBase
{
	public static String getTestCaseName()
	{
		return "Person  RESTful Resource Test Case";
	}
	...
	public void testGet() throws Exception
	{
	  ...
	  try {
		...
		{
			Person obj = (Person) ((List)results).get(0);
		...
		String url = baseURL + "/rest/Person/"+id;
		...
		if (response.getStatus() == Status.NOT_ACCEPTABLE.getStatusCode()) {
 		...
 		File myFile = new File("Person"+"XML.xml");	
	...
	public void testSearch() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/Person/search;id=*";
		WebClient client = WebClient.create(url);
		client.type("application/xml").accept("application/xml");		
		Response response = client.get();
		...
		if (response.getStatus() == Status.NOT_ACCEPTABLE.getStatusCode()) {
 		...
 		File myFile = new File("Person_Search"+"XML.xml");						
		System.out.println("writing data to file "+myFile.getAbsolutePath());
		FileWriter myWriter = new FileWriter(myFile);
...
//***************************************************
	...
	public void testPerson1()
	{
		...
		String url = baseURL + "/rest/Person/search;id=*/livesAt";
	...
	public void testgetLivesAt()
	{
		try
		{
		Person searchObject = new Person();
 		Collection results5 = getApplicationService().search("gov.nih.nci.cacoresdk.domain.onetoone.unidirectional.Person",searchObject );
		String id = "";
		
		if(results5 != null && results5.size() > 0)
		{
			Person obj = (Person) ((List)results5).get(0);
		...
		if(id.equals(""))
			...
			String url = baseURL + "/rest/Person/"+id+"/livesAt";
			WebClient client = WebClient.create(url);
			client.type("application/xml").accept("application/xml");		
			Response response = client.get();
...
//********************************************************End
	...
	public void testPost() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/Person";
		WebClient client = WebClient.create(url);
		...
		File myFile = new File("Person"+"XML.xml");						
 		...
 		{
 			testGet();
 			myFile = new File("Person"+"XML.xml");
 			if(!myFile.exists())
				return;
 		}
