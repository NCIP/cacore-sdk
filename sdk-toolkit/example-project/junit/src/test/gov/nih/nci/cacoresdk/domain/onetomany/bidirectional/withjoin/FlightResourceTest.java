...
public class FlightResourceTest extends SDKRESTfulTestBase
{
	public static String getTestCaseName()
	{
		return "Flight  RESTful Resource Test Case";
	}
	...
	public void testGet() throws Exception
	{
	  ...
	  try {
		...
		{
			Flight obj = (Flight) ((List)results).get(0);
		...
		String url = baseURL + "/rest/Flight/"+id;
		...
		if (response.getStatus() == Status.NOT_ACCEPTABLE.getStatusCode()) {
 		...
 		File myFile = new File("Flight"+"XML.xml");	
	...
	public void testSearch() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/Flight/search;id=*";
		WebClient client = WebClient.create(url);
		client.type("application/xml").accept("application/xml");		
		Response response = client.get();
		...
		if (response.getStatus() == Status.NOT_ACCEPTABLE.getStatusCode()) {
 		...
 		File myFile = new File("Flight_Search"+"XML.xml");						
		System.out.println("writing data to file "+myFile.getAbsolutePath());
		FileWriter myWriter = new FileWriter(myFile);
...
//***************************************************
	...
	public void testFlight1()
	{
		...
		String url = baseURL + "/rest/Flight/search;id=*/passangerCollection";
	...
	public void testgetPassangerCollection()
	{
		try
		{
		Flight searchObject = new Flight();
 		Collection results4 = getApplicationService().search("gov.nih.nci.cacoresdk.domain.onetomany.bidirectional.withjoin.Flight",searchObject );
		String id = "";
		
		if(results4 != null && results4.size() > 0)
		{
			Flight obj = (Flight) ((List)results4).get(0);
		...
		if(id.equals(""))
			...
			String url = baseURL + "/rest/Flight/"+id+"/passangerCollection";
...
//********************************************************End
	...
	public void testPost() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/Flight";
		WebClient client = WebClient.create(url);
		...
		File myFile = new File("Flight"+"XML.xml");						
 		...
 		{
 			testGet();
 			myFile = new File("Flight"+"XML.xml");
 			if(!myFile.exists())
				return;
 		}
