...
public class HandResourceTest extends SDKRESTfulTestBase
{
	...
	public void testSearch() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/Hand/search;id=*";
		WebClient client = WebClient.create(url);
		client.type("application/xml").accept("application/xml");		
		Response response = client.get();
...
//***************************************************
	...
	public void testgetCardCollection()
	{
		...
		{
		Hand searchObject = new Hand();
 		Collection results4 = getApplicationService().search("gov.nih.nci.cacoresdk.domain.other.levelassociation.Hand",searchObject );
		String id = "";
		
		if(results4 != null && results4.size() > 0)
		{
			Hand obj = (Hand) ((List)results4).get(0);
		...
		if(id.equals(""))
			...
			String url = baseURL + "/rest/Hand/"+id+"/cardCollection";
