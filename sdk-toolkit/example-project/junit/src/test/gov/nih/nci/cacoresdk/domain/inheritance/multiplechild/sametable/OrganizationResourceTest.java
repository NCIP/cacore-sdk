...
public class OrganizationResourceTest extends SDKRESTfulTestBase
{
	public static String getTestCaseName()
	{
		return "Organization  RESTful Resource Test Case";
	}
	...
	public void testGet() throws Exception
	{
	  ...
	  try {
		...
		{
			Organization obj = (Organization) ((List)results).get(0);
		...
		String url = baseURL + "/rest/Organization/"+id;
		...
		if (response.getStatus() == Status.NOT_ACCEPTABLE.getStatusCode()) {
 		...
 		File myFile = new File("Organization"+"XML.xml");	
	...
	public void testSearch() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/Organization/search;id=*";
		WebClient client = WebClient.create(url);
		client.type("application/xml").accept("application/xml");		
		Response response = client.get();
		...
		if (response.getStatus() == Status.NOT_ACCEPTABLE.getStatusCode()) {
 		...
 		File myFile = new File("Organization_Search"+"XML.xml");						
		System.out.println("writing data to file "+myFile.getAbsolutePath());
		FileWriter myWriter = new FileWriter(myFile);
...
//********************************************************End
	...
	public void testPost() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/Organization";
		WebClient client = WebClient.create(url);
		...
		File myFile = new File("Organization"+"XML.xml");						
 		...
 		{
 			testGet();
 			myFile = new File("Organization"+"XML.xml");
 			if(!myFile.exists())
				return;
 		}
