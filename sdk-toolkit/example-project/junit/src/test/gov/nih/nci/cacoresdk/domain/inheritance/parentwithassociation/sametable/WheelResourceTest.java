...
public class WheelResourceTest extends SDKRESTfulTestBase
{
	...
	{
		return "Wheel  RESTful Resource Test Case";
	}
	...
	public void testGet() throws Exception
	{
	  ...
	  try {
		...
		{
			Wheel obj = (Wheel) ((List)results).get(0);
		...
		String url = baseURL + "/rest/Wheel/"+id;
		...
		if (response.getStatus() == Status.NOT_ACCEPTABLE.getStatusCode()) {
 		...
 		File myFile = new File("Wheel"+"XML.xml");	
	...
	public void testSearch() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/Wheel/search;id=*";
		WebClient client = WebClient.create(url);
		client.type("application/xml").accept("application/xml");		
		Response response = client.get();
...
//***************************************************
	...
	public void testgetLuggage()
	{
		...
		{
		Wheel searchObject = new Wheel();
 		Collection results4 = getApplicationService().search("gov.nih.nci.cacoresdk.domain.inheritance.parentwithassociation.sametable.Wheel",searchObject );
		String id = "";
		
		if(results4 != null && results4.size() > 0)
		{
			Wheel obj = (Wheel) ((List)results4).get(0);
		...
		if(id.equals(""))
			...
			String url = baseURL + "/rest/Wheel/"+id+"/luggage";
