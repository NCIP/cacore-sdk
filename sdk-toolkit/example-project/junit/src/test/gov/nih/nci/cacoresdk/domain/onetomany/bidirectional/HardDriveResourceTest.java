...
public class HardDriveResourceTest extends SDKRESTfulTestBase
{
	public static String getTestCaseName()
	{
		return "HardDrive  RESTful Resource Test Case";
	}
	...
	public void testGet() throws Exception
	{
	  ...
	  try {
		...
		{
			HardDrive obj = (HardDrive) ((List)results).get(0);
		...
		String url = baseURL + "/rest/HardDrive/"+id;
		...
		if (response.getStatus() == Status.NOT_ACCEPTABLE.getStatusCode()) {
 		...
 		File myFile = new File("HardDrive"+"XML.xml");	
	...
	public void testSearch() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/HardDrive/search;id=*";
		WebClient client = WebClient.create(url);
		client.type("application/xml").accept("application/xml");		
		Response response = client.get();
		...
		if (response.getStatus() == Status.NOT_ACCEPTABLE.getStatusCode()) {
 		...
 		File myFile = new File("HardDrive_Search"+"XML.xml");						
		System.out.println("writing data to file "+myFile.getAbsolutePath());
		FileWriter myWriter = new FileWriter(myFile);
...
//***************************************************
	...
	public void testHardDrive1()
	{
		try
		{
	
		String url = baseURL + "/rest/HardDrive/search;id=*/computer";
	...
	public void testgetComputer()
	{
		...
		{
		HardDrive searchObject = new HardDrive();
 		Collection results5 = getApplicationService().search("gov.nih.nci.cacoresdk.domain.onetomany.bidirectional.HardDrive",searchObject );
		String id = "";
		
		if(results5 != null && results5.size() > 0)
		{
			HardDrive obj = (HardDrive) ((List)results5).get(0);
		...
		if(id.equals(""))
			...
			String url = baseURL + "/rest/HardDrive/"+id+"/computer";
			WebClient client = WebClient.create(url);
			client.type("application/xml").accept("application/xml");		
			Response response = client.get();
...
//********************************************************End
	...
	public void testPost() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/HardDrive";
		WebClient client = WebClient.create(url);
		...
		File myFile = new File("HardDrive"+"XML.xml");						
 		...
 		{
 			testGet();
 			myFile = new File("HardDrive"+"XML.xml");
 			if(!myFile.exists())
				return;
 		}
