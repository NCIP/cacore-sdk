...
public class KeyChainResourceTest extends SDKRESTfulTestBase
{
	public static String getTestCaseName()
	{
		return "KeyChain  RESTful Resource Test Case";
	}
	...
	public void testGet() throws Exception
	{
	  ...
	  try {
		...
		{
			KeyChain obj = (KeyChain) ((List)results).get(0);
		...
		String url = baseURL + "/rest/KeyChain/"+id;
		...
		if (response.getStatus() == Status.NOT_ACCEPTABLE.getStatusCode()) {
 		...
 		File myFile = new File("KeyChain"+"XML.xml");	
	...
	public void testSearch() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/KeyChain/search;id=*";
		WebClient client = WebClient.create(url);
		client.type("application/xml").accept("application/xml");		
		Response response = client.get();
		...
		if (response.getStatus() == Status.NOT_ACCEPTABLE.getStatusCode()) {
 		...
 		File myFile = new File("KeyChain_Search"+"XML.xml");						
		System.out.println("writing data to file "+myFile.getAbsolutePath());
		FileWriter myWriter = new FileWriter(myFile);
...
//***************************************************
	...
	public void testKeyChain1()
	{
		try
		{
	
		String url = baseURL + "/rest/KeyChain/search;id=*/keyCollection";
	...
	public void testgetKeyCollection()
	{
		try
		{
		KeyChain searchObject = new KeyChain();
 		Collection results4 = getApplicationService().search("gov.nih.nci.cacoresdk.domain.onetomany.unidirectional.KeyChain",searchObject );
		String id = "";
		
		if(results4 != null && results4.size() > 0)
		{
			KeyChain obj = (KeyChain) ((List)results4).get(0);
		...
		if(id.equals(""))
			...
			String url = baseURL + "/rest/KeyChain/"+id+"/keyCollection";
...
//********************************************************End
	...
	public void testPost() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/KeyChain";
		WebClient client = WebClient.create(url);
		...
		File myFile = new File("KeyChain"+"XML.xml");						
 		...
 		{
 			testGet();
 			myFile = new File("KeyChain"+"XML.xml");
 			if(!myFile.exists())
				return;
 		}
