...
public class PassangerResourceTest extends SDKRESTfulTestBase
{
	public static String getTestCaseName()
	{
		return "Passanger  RESTful Resource Test Case";
	}
	...
	public void testGet() throws Exception
	{
	  ...
	  try {
		...
		{
			Passanger obj = (Passanger) ((List)results).get(0);
		...
		String url = baseURL + "/rest/Passanger/"+id;
		...
		if (response.getStatus() == Status.NOT_ACCEPTABLE.getStatusCode()) {
 		...
 		File myFile = new File("Passanger"+"XML.xml");	
	...
	public void testSearch() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/Passanger/search;id=*";
		WebClient client = WebClient.create(url);
		client.type("application/xml").accept("application/xml");		
		Response response = client.get();
		...
		if (response.getStatus() == Status.NOT_ACCEPTABLE.getStatusCode()) {
 		...
 		File myFile = new File("Passanger_Search"+"XML.xml");						
		System.out.println("writing data to file "+myFile.getAbsolutePath());
		FileWriter myWriter = new FileWriter(myFile);
...
//***************************************************
	...
	public void testPassanger1()
	{
		try
		{
	
		String url = baseURL + "/rest/Passanger/search;id=*/flight";
	...
	public void testgetFlight()
	{
		...
		{
		Passanger searchObject = new Passanger();
 		Collection results5 = getApplicationService().search("gov.nih.nci.cacoresdk.domain.onetomany.bidirectional.withjoin.Passanger",searchObject );
		String id = "";
		
		if(results5 != null && results5.size() > 0)
		{
			Passanger obj = (Passanger) ((List)results5).get(0);
		...
		if(id.equals(""))
			...
			String url = baseURL + "/rest/Passanger/"+id+"/flight";
			WebClient client = WebClient.create(url);
			client.type("application/xml").accept("application/xml");		
			Response response = client.get();
...
//********************************************************End
	...
	public void testPost() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/Passanger";
		WebClient client = WebClient.create(url);
		...
		File myFile = new File("Passanger"+"XML.xml");						
 		...
 		{
 			testGet();
 			myFile = new File("Passanger"+"XML.xml");
 			if(!myFile.exists())
				return;
 		}
