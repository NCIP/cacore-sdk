...
public class RestaurantResourceTest extends SDKRESTfulTestBase
{
	public static String getTestCaseName()
	{
		return "Restaurant  RESTful Resource Test Case";
	}
	...
	public void testGet() throws Exception
	{
	  ...
	  try {
		...
		{
			Restaurant obj = (Restaurant) ((List)results).get(0);
		...
		String url = baseURL + "/rest/Restaurant/"+id;
		...
		if (response.getStatus() == Status.NOT_ACCEPTABLE.getStatusCode()) {
 		...
 		File myFile = new File("Restaurant"+"XML.xml");	
	...
	public void testSearch() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/Restaurant/search;id=*";
		WebClient client = WebClient.create(url);
		client.type("application/xml").accept("application/xml");		
		Response response = client.get();
		...
		if (response.getStatus() == Status.NOT_ACCEPTABLE.getStatusCode()) {
 		...
 		File myFile = new File("Restaurant_Search"+"XML.xml");						
		System.out.println("writing data to file "+myFile.getAbsolutePath());
		FileWriter myWriter = new FileWriter(myFile);
...
//********************************************************End
	...
	public void testPost() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/Restaurant";
		WebClient client = WebClient.create(url);
		...
		File myFile = new File("Restaurant"+"XML.xml");						
 		...
 		{
 			testGet();
 			myFile = new File("Restaurant"+"XML.xml");
 			if(!myFile.exists())
				return;
 		}
