...
public class DesignerShoesResourceTest extends SDKRESTfulTestBase
{
	public static String getTestCaseName()
	{
		return "DesignerShoes  RESTful Resource Test Case";
	}
	...
	public void testGet() throws Exception
	{
	  ...
	  try {
		...
		{
			DesignerShoes obj = (DesignerShoes) ((List)results).get(0);
		...
		String url = baseURL + "/rest/DesignerShoes/"+id;
		...
		if (response.getStatus() == Status.NOT_ACCEPTABLE.getStatusCode()) {
 		...
 		File myFile = new File("DesignerShoes"+"XML.xml");	
	...
	public void testSearch() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/DesignerShoes/search;id=*";
		WebClient client = WebClient.create(url);
		client.type("application/xml").accept("application/xml");		
		Response response = client.get();
...
//***************************************************
	...
	public void testgetDesigner()
	{
		...
		{
		DesignerShoes searchObject = new DesignerShoes();
 		Collection results5 = getApplicationService().search("gov.nih.nci.cacoresdk.domain.inheritance.childwithassociation.sametable.DesignerShoes",searchObject );
		String id = "";
		
		if(results5 != null && results5.size() > 0)
		{
			DesignerShoes obj = (DesignerShoes) ((List)results5).get(0);
		...
		if(id.equals(""))
			...
			String url = baseURL + "/rest/DesignerShoes/"+id+"/designer";
			WebClient client = WebClient.create(url);
			client.type("application/xml").accept("application/xml");		
			Response response = client.get();
...
//********************************************************End
	...
	public void testPost() throws Exception
	{
	  ...
	  try {
		...
		String url = baseURL + "/rest/DesignerShoes";
		WebClient client = WebClient.create(url);
		...
		File myFile = new File("DesignerShoes"+"XML.xml");						
 		...
 		{
 			testGet();
 			myFile = new File("DesignerShoes"+"XML.xml");
 			if(!myFile.exists())
				return;
 		}
