...
CREATE OR REPLACE TRIGGER "LOG_MESSAGE" BEFORE INSERT ON "LOG_MESSAGE" FOR EACH ROW
DECLARE last_Sequence NUMBER; last_InsertID NUMBER; BEGIN IF (:NEW."LOG_ID" IS NULL) THEN SELECT "LOG_MESSAGE_0".NEXTVAL INTO :NEW."LOG_ID" FROM DUAL; ELSE SELECT Last_Number-1 INTO last_Sequence FROM User_Sequences WHERE UPPER(Sequence_Name) = UPPER('LOG_MESSAGE_0'); SELECT :NEW."LOG_ID" INTO last_InsertID FROM DUAL; WHILE (last_InsertID > last_Sequence) LOOP SELECT "LOG_MESSAGE_0".NEXTVAL INTO last_Sequence FROM DUAL; END LOOP; END IF; END;
/
...
CREATE OR REPLACE TRIGGER "OBJECT_ATTRIBUTE" BEFORE INSERT ON "OBJECT_ATTRIBUTE" FOR EACH ROW
DECLARE last_Sequence NUMBER; last_InsertID NUMBER; BEGIN IF (:NEW."OBJECT_ATTRIBUTE_ID" IS NULL) THEN SELECT "OBJECT_ATTRIBUTE_0".NEXTVAL INTO :NEW."OBJECT_ATTRIBUTE_ID" FROM DUAL; ELSE SELECT Last_Number-1 INTO last_Sequence FROM User_Sequences WHERE UPPER(Sequence_Name) = UPPER('OBJECT_ATTRIBUTE_0'); SELECT :NEW."OBJECT_ATTRIBUTE_ID" INTO last_InsertID FROM DUAL; WHILE (last_InsertID > last_Sequence) LOOP SELECT "OBJECT_ATTRIBUTE_0".NEXTVAL INTO last_Sequence FROM DUAL; END LOOP; END IF; END;
/
